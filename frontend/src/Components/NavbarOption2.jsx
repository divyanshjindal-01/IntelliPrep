...
const NavbarOption2 = () => {
  ...
  // 🔹 Logout with redirect to public home
  const handleLogout = async () => {
    try {
      await signOut(auth);
      navigate("/", { replace: true }); // replace = remove from history
    } catch (error) {
  ...
  const navItems = ["Hero", "Dashboard", "Practice Zone", "Community"];
  ...
  return (
    <nav className="bg-[#0A001F] border-b border-[#25004A] px-4 sm:px-6 lg:px-10 py-3 fixed w-full z-50">
      <div className="flex items-center justify-between">
        <Link
          to="/"
          className="text-2xl sm:text-3xl font-bold text-[#D8B8FF] flex items-center gap-2"
        >
          ...
          {navItems.map((section) => (
            <li key={section}>
              <button
                ...
                className="text-[#D8B8FF] hover:text-[#FF6EFF] transition cursor-pointer"
              >
        ...
        {/* Mobile Menu Button */}
        <button
          className="md:hidden text-[#D8B8FF]"
